...
const captchaAnswer = ref(''); // Estado para la respuesta del usuario
const errors = ref({ email: '', match: '', captcha: '' });

const handleRegister = () => {
  // Limpiamos errores al intentar enviar
  errors.value.email = "";
  errors.value.match = "";
  errors.value.captcha = "";

  // 1. Validación de Formato (Email) 
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email.value)) {
    errors.value.email = "Formato de correo inválido.";
  }

  // 2. Validación de Coherencia (Contraseñas) 
  if (password.value !== confirmPassword.value) {
    errors.value.match = "Las contraseñas no coinciden.";
  }

  // 3. Validación de Usuario Humano 
  // Comprobamos que la respuesta sea exactamente 8
  if (parseInt(captchaAnswer.value) !== 8) {
    errors.value.captcha = "Respuesta incorrecta. Inténtalo de nuevo.";
    return; // Detiene el proceso si falla el CAPTCHA
  }

  // Si no hay errores, procedemos
  if (!errors.value.email && !errors.value.match) {
    alert("¡Validación exitosa! Eres un humano y tus datos son correctos."); 
  }
};
</script>